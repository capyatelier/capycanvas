...
| 2. Launch, live canvas under header, idle scheduling and basic input | Physical app launches. Simulator launch/geometry capture passes. Physical Pencil, undo/redo and lifecycle checks remain. | Launch/render checks, platform UI test and working native input remain. |
| 3. Complete input contract and bounded transport | Coalesced/predicted input foundation exists; corrections, sensors, palm/navigation, interruption and real Pencil evidence remain. | Tablet, mouse, trackpad, keyboard, proximity, interruption and physical input evidence remain. |
...
| 6. Progressive visual acceptance for every editor component/state | Matching initial simulator/Chrome capture and full pixel report exist; baseline fails parity. Device captures and complete fixture matrix remain. | Matching native Mac/Chrome captures and complete fixture matrix remain. |
...
The shared host extraction has seven passing host tests and an Android ARM64
...
incomplete layer panel still need work. No Mac visual result has been accepted.
