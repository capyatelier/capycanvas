...
| Toolbar/menu/panel definitions | GTK and JS separately enumerated tools/menus; JS repeated panel names and six-tile count | Shared typed `UiCatalog` and `MENUS`; `TOOLBAR_CONTROLS` supplies initial defaults. Both hosts render dynamic panel/tile views, with allocation, context menus and picker policy owned by Rust; see [customization](../ui/panel-customization.md). |
