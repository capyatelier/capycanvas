...
Integration with the current main branch preserves application file opening
before a canvas exists, multi-image import with remembered picker folders, and
the newer Apple color-command availability. The shared profile picker and manager
also work on the file-launch window. The menu contract test now matches the
accepted grouping of Proof Setup with its viewing toggles.

The merged workspace check and release GTK build pass. Shared suites pass 82 core, 75 color (seven
optional cases ignored), 63 engine and 436 UI tests. Eight isolated native GTK
workflows pass: profile portability, profile picker, proof cancellation, proof
save/reopen/export, application file launch, source import/paste/history, menus,
and preferences/shortcuts. Integration logs are under
`artifacts/color-m3/integration/`.
