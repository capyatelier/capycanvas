...
| Surface | Current source behavior | Required work |
| --- | --- | --- |
| Manage Workspaces: all row bodies and narrow left grips | Shared [`WorkspaceSwitcherRows.swift`](../../apps/layer-apple/Shared/Editor/WorkspaceSwitcherRows.swift) and AppKit/UIKit `NativeReorderInput` retain the real device and contact. | Mouse bodies and all grips are immediate; touch/pen bodies hold before dragging. AppKit mouse/tablet events and UIKit simulator workflows cover pickup, menu continuation, long-list scrolling and persisted order. The user also confirmed these physical Pencil row gestures, with no stray selections or lag. Native Apple menu presentation is the requested next refinement. Order is an application preference, outside workspace layout history. |
