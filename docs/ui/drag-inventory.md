...
| Surface | Current source behavior | Required work |
| --- | --- | --- |
| Drawing tabs and compact drawing selector | [`DrawingTabs.h`](../../apps/layer-windows/DrawingTabs.h) retains native tab widgets and reuses `WorkspaceRowDrag` for selector rows. Shared `DocumentTabs` validates membership/drop targets and owns order history. | Tab bodies and row grips drag immediately after native slop. Selector bodies use immediate mouse pickup and touch/pen hold arbitration, preserving scrolling and menus. Escape, capture loss and canceled contacts cancel; order history is separate from artwork history. Synthetic mouse/touch/pen strip and selector-grip cancellation/reorder/history, keyboard selection and close cancellation passed `exercise-hdr.ps1`. Physical-device acceptance remains open. |
