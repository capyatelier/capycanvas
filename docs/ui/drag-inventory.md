...
| Layer-row bodies | [`LayerPanel.swift`](../../apps/layer-apple/Shared/Editor/LayerPanel.swift) attaches the reorder `DragGesture` only to the grip; body/name/thumbnail targets select or open context actions. | **Gap:** add immediate mouse body dragging and hold-then-drag continuation for touch/pen bodies to meet the shared whole-row convention. A context menu alone is not a body drag path. |
