...
| R7 | **Integration and delivery.** The startup-progress milestone follows `4b1837a`, removing callback-dependent scheduling and obsolete tests while adding a real Metal regression. Both final Releases build without warnings and complete the affected physical workload. Review artwork is preserved; main was fetched and current. Evidence and publication verification are under `artifacts/apple-startup-stall-v1/`. | Integrate later main changes and repeat only checks affected by subsequent fixes. Publish completed milestones and verify the remote revision. The final release and overall goal still require R1–R6. |
