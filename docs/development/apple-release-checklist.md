...
Next are profile/depth editing, the remaining photo workflows, managed display
...
| R7 | **Integration and delivery.** The visual and SDR foundation milestones plus creation/tagged-paint milestone `9e3d2567` are published. The next grouped milestone adds retained photo Open/Place/Paste, shared profile/depth policies and a reproduced macOS memory-budget correction; both Release builds and scoped Metal/shared/Swift checks pass. Physical UIKit workflows and SDR requalification remain open. Full iPad XCTest is blocked by its extra runner's free-profile app limit; preserve artist apps/drawings. Evidence: `artifacts/apple-photo-workflows-v1/`. | Complete profile/depth editing, remaining photo/ICC/export workflows, managed display policy and physical SDR acceptance. The final goal still requires R2–R6 and the new feature scope. |
