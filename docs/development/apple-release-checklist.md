...
| R6 | **Sustained hardware performance and measurement.** SDR layered-4K completes ten minutes with 1.016% long intervals on Mac after upload accounting, and 0.576% on iPad after native tile content reuse. Artwork/previews, nominal thermals, prompt idle and absence of errors/rejected input/missing measured callbacks are verified. Cache and staging limits remain unchanged. A further display-batch correction removes terminal CPU waits, but removing redundant watercolor composition passes reduces short heavy-watercolor long intervals from 12.803%/28.347% to 3.688%/17.929% on Mac/iPad. The heavy-workload gap remains open. Earlier performance evidence remains separately scoped. | Prioritize the remaining heavy-watercolor GPU/display cost. Preserve passing layered-ink evidence and the accepted rare-miss standard. Finish other current SDR profiles and ProPhoto/U16 qualification, recorder-off resource/storage and idle/resume checks, instrumentation overhead and physical input-to-display measurement. The retained footprint curves do not justify a memory workaround. See [Performance](../../apps/layer-apple/PERFORMANCE.md#watercolor-composition-passes--2026-09-16); CPU/GPU/presentation proxies are not physical latency. |
