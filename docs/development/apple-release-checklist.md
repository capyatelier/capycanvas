...
| R6 | **Sustained hardware performance and measurement.** SDR layered-4K completes ten minutes with 1.016% long intervals on Mac after upload accounting, and 0.576% on iPad after native tile content reuse. Artwork/previews, nominal thermals, prompt idle and absence of errors/rejected input/missing measured callbacks are verified. Cache and staging limits remain unchanged. A further display-batch correction removes terminal CPU waits, but removing redundant watercolor composition passes reduces short heavy-watercolor long intervals from 12.803%/28.347% to 3.688%/17.929% on Mac/iPad. Shared render-pass batching further lowers these to 1.764%/15.893%; disabling optional GPU queue timing gives 2.082%/15.740%. Batching independent tile-preservation copies lowers the matching timer-off runs further to 1.030%/12.868%, with exact replay pixels and simpler code. The iPad heavy-workload gap remains open. Earlier performance evidence remains separately scoped. | Prioritize the remaining heavy-watercolor GPU/display cost. Preserve passing layered-ink evidence and the accepted rare-miss standard. Finish other current SDR profiles and ProPhoto/U16 qualification, recorder-off resource/storage and idle/resume checks, instrumentation overhead and physical input-to-display measurement. The retained footprint curves do not justify a memory workaround. See [Performance](../../apps/layer-apple/PERFORMANCE.md#watercolor-tile-copy-batching--2026-09-16); CPU/GPU/presentation proxies are not physical latency. |
