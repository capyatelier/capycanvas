...
Next are the remaining color/photo workflows, managed display integration and physical acceptance. Earlier
...
| R7 | **Integration and delivery.** The visual milestone `c6e616f5` and native SDR foundation `00d27818` are published. The next grouped milestone adds New Drawing options, tagged paint/palettes and a reproduced Metal capability fix. Both final Release builds, 56 Mac Metal bridge checks, shared/owner workflows and native Mac UI pass; physical iPad startup/short synthetic painting passes. Full UIKit form interaction is blocked by the test runner's free-profile app limit. Artist review apps/drawings remain preserved. Evidence: `artifacts/apple-sdr-controls-v1/` and `artifacts/apple-sdr-workflows-v1/`. | Implement the remaining color/photo workflows, managed display policy and physical SDR acceptance. The final goal still requires R2–R6 and the new feature scope. |
