...
| R6 | **Sustained hardware performance and measurement.** The current SDR layered-4K Mac Release completes ten minutes with 1.016% long active intervals, down from 41.397%. The shared correction retains decoded working tiles and charges uploads by bytes within the existing 16 MiB staging ceiling. Artwork/previews, nominal thermals, prompt idle and absence of errors/rejected input/missing measured callbacks are verified. Earlier startup-progress and pre-SDR Mac/iPad results remain separately scoped. | Preserve these results and the accepted rare-miss standard. Finish outstanding current SDR platform/profile and ProPhoto/U16 qualification, recorder-off resource/storage and idle/resume checks, instrumentation overhead and physical input-to-display measurement. The retained footprint curves do not justify a memory workaround. See [Performance](../../apps/layer-apple/PERFORMANCE.md#layered-sdr-drawing-upload-cache-correction--2026-09-16); CPU/GPU/presentation proxies are not physical latency. |
