...
pre-migration algorithms on the tested Mac. The sampled images match exactly;
the complete images differ in thirteen pixels, with no visible difference in
the reviewed artwork. No production renderer or reference tolerance changes
follow from this comparison. The ordinary Linux-reference test still fails.
