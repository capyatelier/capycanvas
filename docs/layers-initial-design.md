...
Status: GTK layer/masking review build implemented; the broader workflow roadmap below is not all shipped. Updated 2026-09-09. These decisions supersede the earlier automatic flat-color-layer proposal. Evidence and screenshot limitations are recorded in [Layers research](layers-research.md).

### Current GTK review build
...
Ungrouping a masked, translucent, or specially blended group is not necessarily appearance-preserving. Reject unsafe ungroup/merge cases with an explanation rather than discard effects. Initially support simple Normal merges, whole clipping-stack baking, and isolated-group flattening when representable. Broader merge semantics can follow. Every destructive command is undoable and clearly named.
