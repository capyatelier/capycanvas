...
The app currently starts with an embedded copy of these **same** resources,
generated by a directory-scanning build step (no filter list in Rust). External
package loading works at the core/renderer boundary, but production hosts still
need startup resource loading and the transactional live-publication API. Do not
interpret the embedded fallback as completion of the no-rebuild host workflow.

Still required: asynchronous device compilation/rollback before publication,
cross-package shader naming checks, preserving compatible live parameter values,
catalog/property/preview refresh across GTK/web/Android, resource packaging for
the hosts, and final before/after cross-platform timing checks. Curves and Gradient
Map remain the agreed Rust exceptions. No shader-editor UI is being added.
