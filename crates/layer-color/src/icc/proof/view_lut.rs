...
impl ProofLut {
    ...
    /// coverage. Transparent pixels never show a warning. This CPU path is also
    /// the reference for managed UI patches and the presenter shader.
    pub fn apply_premultiplied(&self, rgba: [f32; 4], proof: bool, warnings: bool) -> [f32; 4] {
        let alpha = rgba[3];
        if alpha <= 0. || (!proof && !warnings) {
            return rgba;
        }
        let encoded: [f32; 3] =
            std::array::from_fn(|i| self.space.encode(f64::from(rgba[i] / alpha)) as f32);
        let outside = encoded.iter().any(|v| !(0. ..=1.).contains(v));
        let bounded = encoded.map(|v| v.clamp(0., 1.));
        let Ok(sample) = self.sample(bounded) else {
            return rgba;
        };
        let mut linear = if proof {
            [sample[0], sample[1], sample[2]]
        } else {
            [rgba[0] / alpha, rgba[1] / alpha, rgba[2] / alpha]
        };
        if warnings && (outside || gamut_score(sample) > 5.) {
            // A neutral warning has the same meaning in every managed space.
            linear = [0.5; 3];
        }
        [
            linear[0] * alpha,
...
mod tests {
    ...
    fn preview_is_bounded_cancellable_and_preserves_alpha() {
        ...
        for alpha in [0., f32::MIN_POSITIVE, 0.001, 0.125, 0.5, 1.] {
            ...
            assert_eq!(output[3].to_bits(), alpha.to_bits());
            assert_eq!(lut.apply_premultiplied(input, false, false), input);
            if alpha > 0. {
                for i in 0..3 {
                    assert!((output[i] - input[i]).abs() <= 0.0002 * alpha);
                }
            }
        }
