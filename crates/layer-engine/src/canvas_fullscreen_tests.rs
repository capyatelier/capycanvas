    ...
    for rate in [200., 240.] {
        for zoom in [1., 2.] {
            for width in [4., 64., 128.] {
                for algorithm in [crate::PredictionAlgorithm::Trajectory] {
                    ...
                    for frame in 1..96 {
                        ...
                        if estimate.source == TipSource::Engine {
                            ...
                            assert!(
                                ...
                                "{algorithm:?} frame={frame} brush={width} rendered={rendered:?} estimate={estimate:?}"
                            );
                        }
                        if now > 0.2 {
                    ...
                    assert!(
                        worst_jump < 8.,
                        "{algorithm:?} brush={width} rate={rate} zoom={zoom}: {worst_jump}px"
                    );
                    input
                        .push(PenEvent {
                            phase: PenPhase::Up,
