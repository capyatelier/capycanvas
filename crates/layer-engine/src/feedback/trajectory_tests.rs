...
fn point(x: f32, y: f32, time: u32) -> StrokePoint {
    StrokePoint {
        position: Point { x, y },
        elapsed_micros: time,
        pressure: 0.6,
...
fn measured_braking_bounds_prediction_without_tail_filter_lag() {
    for algorithm in ALGORITHMS {
        ...
        for i in 0..70 {
            ...
            let tip = state
                .estimate(&real, &[], i * 4000 + 16_000, IDENTITY, config(algorithm))
                .unwrap();
            if (52..=60).contains(&i) {
                assert!(
            ...
            for i in 0..65 {
                ...
                if i == 52 {
                    assert!(
                        ...
                        "{algorithm:?} h={horizon} 8 ms braking: {:?}",
                        tip.point
                    );
                }
                if (53..=60).contains(&i) {
    ...
    for rate in [60, 120, 240, 480] {
        for (radius, omega, radial_speed) in [(60., 20., 0.), (30., 40., 0.), (60., 20., -20.)] {
            for noise in [0., 0.3] {
                ...
                let mut real = Vec::new();
                let mut states: [PredictionState; 1] =
                    std::array::from_fn(|_| PredictionState::default());
                let mut errors = [0.; 1];
                let mut count = 0;
                for i in 0..rate {
                    let time =
                    ...
                    for (j, algorithm) in ALGORITHMS.into_iter().enumerate() {
                        let tip = states[j]
                            .estimate(&real, &[], time + 16_000, IDENTITY, config(algorithm))
                            .unwrap();
                        if time > 200_000 {
                            errors[j] += surface_distance(tip.point.position, truth, IDENTITY);
                        }
                    }
                    count += usize::from(time > 200_000);
                }
                let mean = errors.map(|e| e / count as f32);
                eprintln!(
                    "{rate}Hz r={radius} w={omega} dr={radial_speed} noise={noise}: {mean:?}"
                );
                assert!(mean[0] < 2., "must retain useful lookahead: {mean:?}");
            }
        }
    }
}
...
fn trajectory_emits_a_curve_preserves_sensors_and_is_frame_independent() {
    ...
    let cfg = config(PredictionAlgorithm::Trajectory);
    let mut state = PredictionState::default();
    let estimate = state
...
fn staircase_pauses_do_not_keep_old_direction_on_restart() {
    for algorithm in ALGORITHMS {
        ...
        for i in 0..120 {
            ...
            let tip = state
                .estimate(&real, &[], i * 4000 + 16000, IDENTITY, config(algorithm))
                .unwrap()
                .point;
            if phase >= 0.072 {
                assert!(
                    surface_distance(tip.position, real.last().unwrap().position, IDENTITY) <= 1.7,
                    "{algorithm:?} leg={leg} phase={phase} tip={tip:?}"
                );
            }
            if leg % 2 == 0 {
                assert!(
                    (tip.position.y - y).abs() < 0.2,
                    "old direction: {algorithm:?} {tip:?}"
                );
            } else {
                assert!(
                    (tip.position.x - x).abs() < 0.2,
                    "old direction: {algorithm:?} {tip:?}"
                );
            }
        }
...
fn trajectory_handles_quantized_times_duplicates_and_distance_caps() {
    let cfg = InstantFeedbackConfig {
        timestamp_resolution_micros: 1_000,
        ..config(PredictionAlgorithm::Trajectory)
    };
    for rate in [60, 120, 240, 480] {
        for truncate in [false, true] {
            ...
            eprintln!(
                "quantized time rate={rate} truncate={truncate} mean={} worst={worst}",
                error / count as f32
            );
...
fn trajectory_length_is_not_quantized_to_curve_sampling_intervals() {
    ...
    for algorithm in [PredictionAlgorithm::Trajectory] {
        ...
        for limit in [10.25, 10.5, 10.75, 11., 11.25] {
            ...
            assert!(
                lead >= limit - 0.002,
                "{algorithm:?}: lead {lead} must follow the {limit} px bound continuously"
            );
