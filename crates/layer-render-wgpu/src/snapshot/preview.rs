...
impl SnapshotRenderer {
    ...
    pub fn preview_document_for_display(
        ...
        self.preview_document_with_coverage(bounds,space,headroom).map(|v|v.0)
    }
    pub fn preview_document_with_coverage(&mut self,bounds:[u32;2],space:RgbSpace,headroom:f32)->Result<(SnapshotPreview,bool),String>{
    ...
    pub fn preview_gainmap_output(&mut self,bounds:[u32;2],space:RgbSpace,headroom:f32,format:layer_color::photo::GainMapFormat,quality:u8,matte:Option<[f32;3]>) -> Result<(SnapshotPreview,SnapshotPreview,layer_color::OutputStatistics),String>{
        let rendition=self.sdr_rendition.ok_or("Gain-map delivery requires HDR artwork")?;
        let control=self.control.clone();let mut result=None;
        let stats=self.hdr_rows(|extent,working,read|{
            let (extent,hdr,sdr,stats)=layer_color::photo::preview_gainmap_rows(extent,bounds,working,rendition,format,quality,matte,control.cancellation_flag(),read)?;
            result=Some((extent,hdr,sdr));Ok(stats)
        })?;
        let (extent,mut hdr,mut sdr)=result.unwrap();
        let matrix=RgbSpace::Srgb.linear_transform(space);let mapper=rendition.mapper(RgbSpace::Srgb,space);
        for p in &mut hdr{
            if headroom<=1.{*p=mapper.map_premultiplied(*p);continue;}
            *p=layer_core::color::hdr::map_display_premultiplied(*p,headroom);
            let rgb=layer_core::color::rgb::apply(matrix,[p[0],p[1],p[2]].map(f64::from));p[..3].copy_from_slice(&rgb.map(|v|v as f32));
        }
        for p in &mut sdr{let rgb=layer_core::color::rgb::apply(matrix,[p[0],p[1],p[2]].map(f64::from));p[..3].copy_from_slice(&rgb.map(|v|v as f32));}
        Ok((SnapshotPreview{extent,space,pixels:hdr},SnapshotPreview{extent,space,pixels:sdr},stats))
    }
