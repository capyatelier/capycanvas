...
impl Scene {
    ...
    pub fn thumbnail(
        ...
        let preview = create_page_surface(
            &r.device,
            &r.texture_layout,
            &r.sampler,
            [32, 32],
        ...
        self.jobs.push(Job::Clear(
            ...
            wgpu::Color {
                ...
                g: gray,
                b: gray,
                a: if mask.is_some() { 1. } else { 0. },
            },
        ));
        let sources: Vec<_> = if let Some(mask) = mask {
            r.layer_masks
                .pages
                .iter()
                .filter(|((target, _), _)| *target == id)
                .map(|((_, c), p)| (*c, p.view.clone(), true, mask.inverted))
                .collect()
        } else {
            r.paint_layers
                .iter()
                ...
                .map(|l| {
                    l.pages
                        .iter()
                        .map(|p| (p.coordinate, p.active().view.clone(), false, false))
                        .collect()
                })
                .unwrap_or_default()
        };
        let scale = 32. / r.document_extent[0].max(r.document_extent[1]).max(1) as f32;
        let y = (32. - r.document_extent[1] as f32 * scale) * 0.5;
        let x = (32. - r.document_extent[0] as f32 * scale) * 0.5;
        for (c, source, scalar, inverted) in sources {
            let mut data = [0.; 16];
            data[..4].copy_from_slice(&[
                x + c[0] as f32 * 256. * scale,
                y + c[1] as f32 * 256. * scale,
                256. * scale,
                256. * scale,
            ]);
            data[4..8].copy_from_slice(&[32., 32., 0., 0.]);
            ...
            self.jobs.push(Job::Draw {
                target: preview.view.clone(),
                sources: [source, r.empty_view.clone()],
                data,
                over: !scalar,
            });
        }
