...
fn original(p:vec2<i32>)->vec4<f32> {
    if any(p<vec2(0)) || any(p>=vec2<i32>(textureDimensions(source))) {return vec4(transform.target_flags.w);}
...
fn transformed(local:vec2<f32>)->vec4<f32> {
    ...
    if any(local<vec2(-.5)) || any(local>vec2<f32>(textureDimensions(source))+vec2(.5)) {
