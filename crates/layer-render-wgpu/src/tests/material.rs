...
fn single_prediction_borrows_coverage_and_survives_private_preview_transitions() {
    ...
    let committed = submit(&mut renderer, &predicted, &[batch], false);
    let maximum = committed.iter().zip(&expected)
        .map(|(a, b)| a.abs_diff(*b)).max().unwrap();
    // Compare displayed previews to displayed committed paint. The persistent
    // sRGB8 attachment adds a quantization boundary; it can change the final
    // display by one code. Preview reuse above remains byte-exact, as does
    // backing/save/reopen coverage in the project tests.
    assert!(maximum <= 1, "committing a preview changed display by {maximum}/255");
    eprintln!(
