//! GPU connected-region primitive. UI integration follows pixel/performance
//! validation; no production tool routes to this module yet.
use super::*;
...
impl Flood {
    fn new(device: &wgpu::Device) -> Self {
        ...
        entries.extend((1..5).map(|binding| wgpu::BindGroupLayoutEntry {
            binding,
            visibility: wgpu::ShaderStages::COMPUTE,
    ...
    for extent in [
        ...
        for pattern in 0..6 {
            ...
            for seed in [[0, 0], [w / 2, h / 2], [w - 1, h - 1]] {
                ...
                let mut encoder = r.device.create_command_encoder(&Default::default());
                let region = flood
                    .encode(&r.device, &mut encoder, &source, extent, seed, 0.)
                    .unwrap();
...
fn tolerance_is_seed_relative_and_includes_transparency() {
    ...
    for pixels in [
        ...
        for (tolerance, expected, count) in [(0.1, 0x44, 2), (0.2, 0x4444, 4)] {
            let mut encoder = r.device.create_command_encoder(&Default::default());
            let region = flood
                .encode(&r.device, &mut encoder, &source, [4, 1], [0, 0], tolerance)
                .unwrap();
            r.queue.submit([encoder.finish()]);
...
fn queued_regions_keep_independent_results_while_reusing_scratch() {
    ...
    for extent in [[65, 49], [19, 17], [65, 49]] {
        ...
        let source = source(&r, extent, &pixels);
        let region = flood
            .encode(&r.device, &mut encoder, &source, extent, [0, 0], 0.)
            .unwrap();
...
fn invalid_region_requests_do_not_allocate_scratch() {
    ...
    for (extent, seed, tolerance) in [
        ...
        assert!(
            flood
                .encode(&r.device, &mut encoder, &source, extent, seed, tolerance)
                .is_err()
        );
    ...
    for pattern in ["solid", "linework", "maze", "noise"] {
        let pixels: Vec<u8> = (0..extent[0] * extent[1])
            .flat_map(|i| {
                ...
                let on = match pattern {
                    ...
                    "maze" => y % 2 == 0 || x == if (y / 2) % 2 == 0 { extent[0] - 1 } else { 0 },
        ...
        for i in 0..150 {
            ...
            if i == 0 {
                ...
                eprintln!("{pattern}: first completion {elapsed:.3}ms; bounds/count {bounds:?}");
            }
        }
