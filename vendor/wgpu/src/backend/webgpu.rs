...
fn pipeline_creation_error(label: &str, error: JsValue) -> String {
    let property = |name: &str| {
        js_sys::Reflect::get(&error, &JsValue::from_str(name))
            .ok()
            .and_then(|value| value.as_string())
    };
    let message = property("message").unwrap_or_else(|| format!("{error:?}"));
    let reason = property("reason").unwrap_or_else(|| "pipeline creation".to_string());
    format!("{label} ({reason}): {message}")
}
...
impl WebDevice {
    pub fn create_compute_pipeline_async(
        &self,
        desc: &crate::ComputePipelineDescriptor<'_>,
    ) -> impl Future<Output = Result<dispatch::DispatchComputePipeline, String>> + 'static {
        let promise = self
            .inner
            .create_compute_pipeline_async(&map_compute_pipeline_descriptor(desc));
        let label = desc.label.unwrap_or("unlabeled pipeline").to_string();
        // Attach rejection handling now, even if another pipeline's future is
        // awaited first or the caller later abandons this compilation.
        let future = wasm_bindgen_futures::JsFuture::from(promise);
        async move {
            let pipeline = future.await
                .map_err(|error| pipeline_creation_error(&label, error))?;
            Ok(WebComputePipeline {
                inner: pipeline,
                ident: crate::cmp::Identifier::create(),
            }
            .into())
    ...
    pub fn create_render_pipeline_async(
        &self,
        desc: &crate::RenderPipelineDescriptor<'_>,
    ) -> impl Future<Output = Result<dispatch::DispatchRenderPipeline, String>> + 'static {
        let promise = self
            .inner
            .create_render_pipeline_async(&map_render_pipeline_descriptor(desc));
        let label = desc.label.unwrap_or("unlabeled pipeline").to_string();
        let future = wasm_bindgen_futures::JsFuture::from(promise);
        async move {
            let pipeline = future.await
                .map_err(|error| pipeline_creation_error(&label, error))?;
            Ok(WebRenderPipeline {
                inner: pipeline,
                ident: crate::cmp::Identifier::create(),
            }
            .into())
