...
pub extern "system" fn Java_art_capycanvas_Native_toneStatus(
    ...
    string(&mut env,Ok(serde_json::json!({"generation":a.tone.generation,"hdr":a.tone.key.is_some(),"ready":a.tone.guide.is_some(),"error":a.tone.error,
        "display_hdr":a.hdr_capable(),"compositor_hdr":a.compositor_hdr(),"display_headroom":a.hdr_headroom(),"reported_headroom":a.display_headroom,"requested_headroom":a.requested_headroom(),"proof_mode":a.host.session.proof_panel_mode(),
