        ...
        loop=host.viewModelScope.launch {
            while(isActive&&!paused){
                try{
                    ...
                    status=if(!state.getBoolean("hdr"))"" else if(!state.isNull("error"))"SDR preview unavailable: ${state.getString("error")}" else if(state.getBoolean("ready"))"HDR artwork · mapped SDR display" else "HDR artwork · preparing SDR preview…"
                    if(state.getBoolean("needed")&&running==null&&android.os.SystemClock.elapsedRealtime()-changed>=180)start(generation)
                }catch(e:CancellationException){throw e}catch(e:Exception){status="SDR preview unavailable: ${e.message}"}
                delay(200)
            }
        }
    }
