...
internal class OutputPreview(private val host:CanvasHost) {
    ...
    fun invalidate(){images=emptyList();error=null;clipped=0}
            ...
            try {
                val result=withContext(NonCancellable) {
                    ...
                    withContext(Dispatchers.IO){val values=Native.inspectionOutput(task,recipe.toString());
                        JSONObject(values[0] as String) to listOf(comparisonBitmap(values[1] as ByteArray),comparisonBitmap(values[2] as ByteArray))}
                }
                if(!closed){clipped=result.first.optLong("clipped_channels");images=result.second}
