#!/usr/bin/env bash
# Workspace menus through actual pointer delivery and native SQLite startup.
set -euo pipefail
if [[ "${1:-}" != --session ]]; then
...
menu_compositor_pid=$!
trap 'kill "$menu_compositor_pid" 2>/dev/null || true; wait "$menu_compositor_pid" 2>/dev/null || true' EXIT
...
G_DEBUG=fatal-criticals gjs apps/layer-linux/bench/native-input.js --workspace-menus \
