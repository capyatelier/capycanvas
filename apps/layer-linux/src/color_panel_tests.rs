...
pub(super) fn hue_guide(w: &Workspace) -> gtk::gdk::MemoryTexture {
    use gtk::subclass::prelude::ObjectSubclassIsExt;
    let wheel = find_named(w.color_panel.root.upcast_ref(), "color-wheel")
        .unwrap()
        .downcast::<crate::tool_panels::ColorWheel>()
        .unwrap();
    let cache = wheel.imp().hue_guide.borrow();
    let (side, shape, texture) = cache.as_ref().expect("visible wheel caches its hue guide");
...
fn assert_hue_guide_colors(w: &Workspace, texture: &gtk::gdk::MemoryTexture) {
