...
// Use only an explicitly selected existing tablet test tab. Open the photograph
// there first. This does not launch Chrome or change its flags/other tabs.
if (process.argv[1] && import.meta.url === (await import('node:url')).pathToFileURL(process.argv[1]).href) {
  const [, , tabId, output] = process.argv;
  assert.ok(tabId && output, 'Usage: node photo-navigation-bench.test.mjs TEST_TAB_ID REPORT.json');
  const endpoint = process.env.LAYER_CDP_URL || 'http://127.0.0.1:9228';
  const tabs = await (await fetch(`${endpoint}/json/list`)).json();
  const tab = tabs.find(t => t.id === tabId);
  assert.ok(tab?.webSocketDebuggerUrl, 'The explicitly selected test tab must exist');
  const socket = new WebSocket(tab.webSocketDebuggerUrl), pending = new Map();
  let sequence = 0;
  ...
  const call = (method, params = {}) => new Promise((resolve, reject) => {
    const id = ++sequence;
    const timer = setTimeout(() => { pending.delete(id); reject(Error(`Timed out: ${method}`)); }, 120000);
  ...
  try {
    ...
    await benchPhotoNavigation({call, evaluate}, output);
  } finally {
