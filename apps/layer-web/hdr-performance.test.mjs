...
export async function measureHdr({call,evaluate,settle}) {
  ...
  const memory=async()=>{
    try {
      if(process.env.LAYER_DEVICE_SERIAL){const {stdout}=await exec(process.env.ADB||'adb',['-s',process.env.LAYER_DEVICE_SERIAL,'shell','dumpsys','meminfo','com.android.chrome']);const pss=[...stdout.matchAll(/TOTAL PSS:\s+(\d+)/g)].reduce((s,m)=>s+Number(m[1])*1024,0);return{pss_bytes:pss||null,source:'ADB dumpsys meminfo com.android.chrome',raw:stdout};}
      const {processInfo}=await call('SystemInfo.getProcessInfo',{},null);let pss=0,observed=0;
      for(const p of processInfo){try{const m=(await readFile(`/proc/${p.id}/smaps_rollup`,'utf8')).match(/^Pss:\s+(\d+)/m);if(m){pss+=Number(m[1])*1024;observed++;}}catch{}}
      return{pss_bytes:pss||null,observed,processes:processInfo.length,source:'Chrome SystemInfo PIDs /proc smaps_rollup'};
  ...
  try {
    for(const name of (process.env.LAYER_HDR_WORKLOADS||'sparse4k,hdr24.png,hdr45.png,hdr60.png').split(',')){
      ...
      try {
        ...
        await wait('layerApp.app.tone_status().ready||layerApp.app.tone_status().error');entry.ready_ms=performance.now()-start;entry.tone=await evaluate(`JSON.parse(JSON.stringify(layerApp.app.tone_status(),(_,v)=>typeof v==='bigint'?Number(v):v))`);assert.equal(entry.tone.error??null,null);entry.cold=await read();if(process.env.LAYER_HDR_PROFILE){const {profile}=await call("Profiler.stop");await writeFile(`${directory}/open-${name}.cpuprofile`,JSON.stringify(profile));await call("Profiler.disable");}
        await invoke('fit_canvas');await invoke('pen');await evaluate(`layerApp.dispatch({type:'select_brush',id:1});layerApp.dispatch({type:'color',action:{op:'set_slot',slot:'foreground',color:{space:'Srgb',rgba:[1.8,.3,.1,1]}}});`);
