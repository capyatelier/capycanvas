...
fn layout_apple_abi_resizes_retained_content_and_matches_legacy_geometry() {
    ...
    fn full_publish(app: &App) -> (Value, usize) {
        unsafe { (*app.0).host.invalidate_snapshot() };
        let (mut value, size) = publish(app, 7).unwrap();
        value.as_object_mut().unwrap().remove("workspace_update");
        (value, size)
    }
    for platform in [0, 1] {
        for floating in [false, true] {
            for cancel in [false, true] {
                ...
                let full = || {
                    let mut old = full_publish(&legacy).0;
                ...
                for step in 1..=16 {
                    ...
                    let (old, old_size) = full_publish(&legacy);
