...
@main struct WorkspaceLibraryChecks {
    @MainActor static func main() async throws {
        for platform: UInt32 in [0, 1] {
            ...
            _ = try await session(["type": "begin"])
            let incoming = try await request(["type": "operation", "operation": ["type": "new", "name": "Fresh"], "now": now + 2])
            let next = try await adopt(incoming)
            precondition(next != original && store.state["brush"]["diameter"].number == 53)
            try await edit(["type": "set_brush_size", "value": 71])
            _ = try await session(["type": "begin"])
            ...
            precondition(view["value"]["rows"].array.count == 4 && !view["value"]["details"]["actions"].array.isEmpty)
