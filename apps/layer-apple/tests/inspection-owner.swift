...
@main struct InspectionOwnerChecks {
    @MainActor static func wait(_ label: String, store: EditorStore, _ ready: () -> Bool) async throws {
        try await CapyTest.wait("\(label); \(store.histogram.status)", failure: { store.failure }, step: { await frame(store.native!) }, ready)
    }
    @MainActor static func main() async throws {
        for platform: UInt32 in [0,1] {
            ...
            try await store.apply(["type":"invoke","command":"histogram"])
            ...
            try await store.apply(["type":"layer","action":["op":"visibility","id":paper,"value":true]])
            ...
            try await store.apply(["type":"invoke","command":"eyedropper"])
            for width in [1,5,15] {
                try await store.apply(["type":"set_color_sample_size","width":width])
