...
@main final class NativeContextSourceChecks: NativeWorkspaceInputFixture {
    @MainActor static func source(_ view: NSView) -> NativeContextMenuInputView? {
        if let source = view as? NativeContextMenuInputView { return source }
        return view.subviews.lazy.compactMap(source).first
    }
    @MainActor static func query(_ editor: EditorStore) async -> JSON {
        await withCheckedContinuation { continuation in
            editor.query(["type": "context", "target": ["kind": "zen_mode"]]) { continuation.resume(returning: $0) }
        }
    }
    ...
    @MainActor static func run(_ platform: UInt32) async throws {
        ...
        let window = NSWindow(contentRect: CGRect(x: 160, y: 160, width: 160, height: 100),
        ...
        let host = NSHostingView(rootView: AnyView(ZStack(alignment: .topLeading) {
            Color.clear; EditorZenButton(store: editor)
        }))
        ...
        let observer = NotificationCenter.default.addObserver(forName: NSMenu.didBeginTrackingNotification,
            ...
            MainActor.assumeIsolated {
                ...
                guard menu.title == "Zen mode" else { return }
                ...
                let timer = Timer(timeInterval: 0.02, repeats: false) { _ in
                    MainActor.assumeIsolated {
                        ...
                        if let index = menu.items.firstIndex(where: { $0.title == "Total zen" && $0.isEnabled }) {
                            menu.performActionForItem(at: index)
                        } else { menuFailure = "Native shared Total zen item missing" }
        ...
        for index in 1...2 {
            ...
            let result = await query(editor)
            try require(result["sections"][0][0]["selected"].bool == (index == 1),
                "The source's native callback must toggle shared Total zen state")
            try await drain()
        }
        ...
        pending?(AppleContextMenu(JSON(["title": "Zen mode", "sections": [[
            ["label": "Total zen", "enabled": true, "action": ["id": "unexpected"]]
        ...
        note("PASS platform \(platform): native source geometry, Rust menu loading, preference toggles and pending-query teardown")
    }
