...
extension XCTestCase {
    @MainActor func checkToolbarStylesAndActions(in app: XCUIApplication) {
        ...
        XCTAssertTrue(editorDocumentTitle(in: app).waitForExistence(timeout: 10))
        XCTAssertTrue(zoom.waitForExistence(timeout: 10))
        XCTAssertEqual(zoom.frame.width, 54, accuracy: 1, "Toolbar style must survive Zen and restoration")
