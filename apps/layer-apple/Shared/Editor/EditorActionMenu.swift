...
struct EditorActionMenu: View {
    ...
    var identifier = "editor-action-menu"
    var dismiss: () -> Void
    @State private var pages: [AppleContextMenu.Item] = []
    ...
    private func itemView(_ item: AppleContextMenu.Item, index: Int) -> some View {
        Button { activate(item) } label: { itemLabel(item) }
    ...
    private func key(_ key: String, command: Bool, shift: Bool, alt: Bool) {
        if let item = entries.first(where: { item in
        ...
        case "arrowright":
            if let focus, entries.indices.contains(focus), !entries[focus].sections.isEmpty { activate(entries[focus]) }
        case "arrowleft": if !pages.isEmpty { back() }
        case "escape": dismiss()
        case "enter", " ":
            if let focus, entries.indices.contains(focus) { activate(entries[focus]) }
        default: break
        }
    }
