...
struct ExportFormView : std::enable_shared_from_this<ExportFormView> {
    ...
    void normalize(J action){
        ...
        profile.IsEnabled(!hdr);intent.IsEnabled(!hdr);blackPoint.IsEnabled(!hdr);quality.IsEnabled(str(recipe,L"format")==L"Jpeg"||str(recipe,L"format")==L"JpegHdr"||str(recipe,L"format")==L"JpegHdrMapped"||str(recipe,L"format")==L"AvifHdr"||str(recipe,L"format")==L"AvifHdrMapped");updating=false;
    }
    J current(){
    ...
    void init(J const& details){
        ...
        blackPoint.Content(box_value(L"Black point compensation"));blackPoint.IsChecked(flag(object(object(recipe,L"encoding"),L"conversion"),L"black_point_compensation"));root.Children().Append(blackPoint);
