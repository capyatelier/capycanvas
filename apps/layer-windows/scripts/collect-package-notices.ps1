...
foreach($line in $tree){
    if($line -notmatch '^(?<name>[\w-]+) v(?<version>[^ ]+)(?: \([^|]*\))?\|(?<license>.+)$'){throw "Unrecognized Cargo dependency: $line"}
    ...
    $source=@($registries|ForEach-Object {Join-Path $_.FullName $key}|Where-Object {Test-Path -LiteralPath $_ -PathType Container})
    if($source.Count -ne 1){throw "Expected one cached registry source for $key"}
    $source=$source[0]
    ...
    if(!$notices.Count){
        switch -Exact ($key){
            'profiling-1.0.18' {Copy-Notice (Join-Path $repo 'apps/layer-windows/packaging/notices/profiling-1.0.18-MIT.txt') "Cargo/$key/LICENSE-MIT"}
    ...
    Copy-Notice (Join-Path $source 'Cargo.toml.orig') "Cargo/$key/Cargo.toml.orig"
    ...
    $dependencies[$key]=[ordered]@{name=$name;version=$version;license=$license;source="https://crates.io/crates/$name/$version"}
}
