param([int]$ProcessId, [string]$Output = 'artifacts/windows/window.png')
...
[CapyWindowCapture]::GetWindowRect($handle,[ref]$rect)|Out-Null
$bitmap=New-Object System.Drawing.Bitmap(($rect.right-$rect.left),($rect.bottom-$rect.top))
$graphics=[System.Drawing.Graphics]::FromImage($bitmap)
$dc=$graphics.GetHdc()
try {
    if(![CapyWindowCapture]::PrintWindow($handle,$dc,2)){throw 'Window capture failed.'}
